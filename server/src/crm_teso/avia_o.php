<?
	class crm_db {
        ...
        var $myconn;
		var $disable = true;
		var $crm_url = "http://localhost:85/svn/crm_bst/";
	...
	class crm {		
		...
		function crm_owner_service_product_xml_list($item) {
			...
			while ($row = mysql_fetch_array($result)) {
				...
				if ($row['deposit'] == 'cash' && $row['product_id'] != 0) $row['type'] == 'cash';
...
//			$this->crm_service_products_to_sales_action($item);
			...
			$result = $this->query_result($q, " WHERE crm_id=".$item->get("where")." ORDER BY $sort desc LIMIT 0,5");
			while ($row = mysql_fetch_array($result)) {
...
//				$row['performTheshold'] = 0;
		...
		function crm_report_customer_list_2($item) {
			...
			$q = "SELECT (select firstname from crm_customer where crm_id=crm_balance.crm_id) as firstname,crm_id,owner FROM crm_balance WHERE owner='$owner' and _date between '$start' and '$end' and crm_id not in (select crm_id from crm_customer where _class='AGENT') group by crm_id";
			$result = $this->query_result($q, "ORDER BY firstname ");
			$sales_total = 0;
			$cash_total = 0;
			$lease_total = 0;
			...
			$soldq = "select firstname,crm_id,owner from crm_customer where owner='$owner' and '$end' and crm_id not in (".implode(',',$sold).") and _class!='AGENT'" ;
			$result = $this->query_result($soldq, "ORDER BY firstname");
			while ($row = mysql_fetch_array($result)) {
				$record = new crm_report_customer();
				$owner = $row['owner'];
				$crm_name = $row['firstname'];
			...
			$r = mysql_fetch_assoc($this->query_result($q,''));
			$json = substr($json, 0, -1);
			return '{"results":'.$count.',"items":['.$json.'], "total":"'.$r['total'].'", "all_total":"'.$sales_total.'","lease_total":"'.$lease_total.'","sold":"'.$soldq.'", }';
		}
		...
		function crm_owner_loan_total_info($owner, $item) {	
			...
			$end_date = $item->get("end_date");
			$count = 0;
			$result = $this->query_result("select sum(amount) as amount  from crm_balance where owner='$owner' and _date between '$start_date' and '$end_date' and deposit='lease' and product_id>0 and crm_id not in (select crm_id from crm_customer where _class='AGENT')", "");
			while ($row = mysql_fetch_array($result)) {
...
//			$where = $this->crm_session_where($where);
		...
		function crm_report_compare_product_user_list($item) {
			...
			$sort = $item->get("sort");
			$dir = $item->get("dir");
			$q = "select product_id,SUM(IF(owner = 'bulgantsetseg@teso', ".$field.",0)) as owner1,SUM(IF(owner = 'nyamsvren@teso', ".$field.",0)) as owner2,SUM(IF(owner = 'enkhtsetseg@teso', ".$field.",0)) as owner3,SUM(IF(owner = 'gantuya@teso', ".$field.",0)) as owner4,SUM(IF(owner = 'gantuul@teso', ".$field.",0)) as owner5,SUM(IF(owner = 'oyunaa@teso', ".$field.",0)) as owner6,SUM(IF(owner = 'altanhuyag@teso', ".$field.",0)) as owner7,SUM(IF(owner = 'erkhembayar@teso', ".$field.",0)) as owner8,SUM(IF(owner = 'altanchimeg@teso', ".$field.",0)) as owner9,SUM(IF(owner = 'munkhjargal@teso', ".$field.",0)) as owner10 ,SUM(IF(owner = 'khoserdene@teso', ".$field.",0)) as owner11,SUM(IF(owner = 'bayartsetseg@teso', ".$field.",0)) as owner12,SUM(IF(owner = 'batsuuri@teso', ".$field.",0)) as owner13,SUM(IF(owner = 'altantsetseg@teso', ".$field.",0)) as owner14,SUM(IF(owner = 'shurentsetseg@teso', ".$field.",0)) as owner15, SUM(IF(owner = 'unurtsetseg@teso', ".$field.",0)) as owner16, SUM(IF(owner = 'ankhzaya@teso', ".$field.",0)) as owner17,SUM(IF(owner ='mungunshagai@teso', ".$field.",0)) as owner18  from crm_balance where deposit='sales' and crm_id not in (select crm_id from crm_customer where _class='AGENT') and _date between '$start_date' and '$end_date' group by product_id";
		...
		function crm_service_products_to_sales_action($item) {			
			...
			while ($row = mysql_fetch_array($result)) {
				...
				while ($row_one = mysql_fetch_array($result_one)) {
					...
					//echo 'service stage '.$service_stage.' qty '.$qty.'  pid '.$product_id.' owner '.$owner;
					/*if($service_stage == "return"){
						$query_five = "update crm_product_available set total=total-$qty where product_id=$product_id and userCode='$owner'";
						mysql_query($query_five, $this->db->conn());
					}*/
