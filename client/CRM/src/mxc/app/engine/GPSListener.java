...
public class GPSListener implements LocationListener  {
    ...
    public void onLocationChanged(Location loc) {
        if (numberOfUpdates < MAX_NUMBER_OF_UPDATES) {
            ...
            if(loc.getLatitude() > 0 && loc.getLongitude() > 0){
            	Shared.currentLat =  loc.getLatitude();
            	Shared.currentLng =  loc.getLongitude();
            }
    ...
    public void onProviderDisabled(String provider) {
        Toast.makeText(activity, "Gps идэвхгүй", Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onProviderEnabled(String provider) {
        Toast.makeText(activity, "Gps идэвхтэй", Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onStatusChanged(String provider, int status, Bundle exWtras) {
